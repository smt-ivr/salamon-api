...
export default {
    async fetch(request, env, ctx) {
        ...
        if (path === '/salamon/style.css') { return new Response(cssContent, { headers: { 'Content-Type': 'text/css; charset=utf-8' } }); }
        if (path === '/salamon/client-main.js') { return new Response(mainJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-auth.js') { return new Response(authJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-messages.js') { return new Response(messagesJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-audio.js') { return new Response(audioJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-settings.js') { return new Response(settingsJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-admin.js') { return new Response(adminJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-admin-advanced.js') { return new Response(adminAdvancedJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-ads.js') { return new Response(adsJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-admin-ads.js') { return new Response(adminAdsJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-admin-chat.js') { return new Response(adminChatJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-user-chat.js') { return new Response(userChatJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        if (path === '/salamon/client-smart-admin.js') { return new Response(smartAdminJs, { headers: { 'Content-Type': 'application/javascript; charset=utf-8' } }); }
        ...
        try {
            ...
            if (pathname.endsWith("/api/phone/stats")) response = await handlePhoneApiStats(request, env);
            ...
            else if (request.method === "POST" && pathname.endsWith("/api/messages/check-delete")) response = await handleCheckDeleteEligibility(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/messages/delete")) response = await handleDeleteMessage(request, env, userIp);
                ...
                if (body.intent === 'reset') {
                    ...
                    if (!body.phone) response = Response.json({ error: "חסר מספר טלפון" }, { status: 400 });
                    else {
            ...
            else if (request.method === "POST" && pathname.endsWith("/api/chat/read")) response = await handleUserMarkRead(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/chat/delete")) response = await handleUserDeleteMessage(request, env);
                ...
                if (request.method !== "POST") response = Response.json({ error: "מתודה לא מורשית" }, { status: 405 });
                    ...
                    if (!adminToken || !adminToken.includes(':')) response = Response.json({ error: "חסר אימות מנהל" }, { status: 401 });
                        ...
                        if (!admin) response = Response.json({ error: "הרשאות מנהל לא חוקיות" }, { status: 403 });
                            ...
                            else if (pathname.endsWith("/api/verify/admin/blocks")) response = Response.json(await verifySystem.getBlocks());
                            else if (pathname.endsWith("/api/verify/admin/block")) response = Response.json(await verifySystem.blockTarget(body.type, body.value, body.reason, body.durationValue, body.durationUnit, userIp));
                            else if (pathname.endsWith("/api/verify/admin/unblock")) response = Response.json(await verifySystem.unblockTarget(body.target, userIp));
                            else if (pathname.endsWith("/api/verify/admin/clean")) response = Response.json(await verifySystem.cleanOldLogs());
                            else response = Response.json({ error: "נתיב ניהול לא נמצא" }, { status: 404 });
                        }
                    }
                }
            }
            ...
            else if (request.method === "POST" && pathname.endsWith("/api/login/google")) response = await handleGoogleLogin(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/user")) response = await handleGetProfile(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/logout")) response = await handleLogout(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/update-profile")) response = await handleUpdateProfile(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/change-password")) response = await handleChangePassword(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/reset-password/confirm")) response = await handleResetPasswordConfirm(request, env);
            else if (request.method === "POST" && pathname.endsWith("/api/unsubscribe/check")) response = await handleCheckUnsubscribeToken(request, env);

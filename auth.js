...
// -------------------------------------------------------------
// פונקציה חדשה: עדכון הגדרה בודדת למשתמשים (Single Auto-save)
// -------------------------------------------------------------
export async function handleUpdateSingleSetting(request, env) {
    const body = await request.json().catch(() => ({}));
    const { userToken, key, value, password } = body;

    if (!userToken) return Response.json({ error: "חסר אימות משתמש (טוקן)" }, { status: 401 });
    if (!key) return Response.json({ error: "חסר מזהה הגדרה (key) לעדכון" }, { status: 400 });

    const user = await authenticateUser(env.DB, userToken);
    if (!user) return Response.json({ error: "הטוקן שגוי או שפג תוקפו, אנא התחבר מחדש" }, { status: 401 });

    // אבטחה: רשימת הגדרות מותרות לעדכון בודד
    const allowedKeys = ['email', 'receive_emails', 'google_login_only'];
    if (!allowedKeys.includes(key)) {
        return Response.json({ 
            success: false, 
            currentValue: user[key] !== undefined ? user[key] : null,
            error: "הגדרה זו אינה קיימת או שאינה מורשית לעדכון מהיר." 
        }, { status: 400 });
    }

    // אימות סיסמה במידה ושיטת ההתחברות דורשת זאת
    if (user.auth_method === 'password') {
        if (!password) {
            return Response.json({ 
                success: false, 
                currentValue: user[key] !== undefined ? (key === 'receive_emails' ? user[key] !== 0 : user[key]) : null,
                error: "חובה להזין את הסיסמה שלך על מנת לשמור שינויים." 
            }, { status: 400 });
        }
        if (user.password !== password) {
            return Response.json({ 
                success: false, 
                currentValue: user[key] !== undefined ? (key === 'receive_emails' ? user[key] !== 0 : user[key]) : null,
                error: "הסיסמה שהוזנה שגויה, לא ניתן לשמור שינויים." 
            }, { status: 401 });
        }
    }
    ...
    // ולידציות ומניפולציות ספציפיות לכל הגדרה בנפרד (על בסיס handleUpdateProfile המקורי שלך)
    if (key === 'receive_emails') {
        finalValue = (value === true || value === 1 || value === 'true' || value === '1') ? 1 : 0;
    } 
    else if (key === 'google_login_only') {
        const intentGoogleOnly = (value === true || value === 1 || value === 'true' || value === '1') ? 1 : 0;
        
        if (intentGoogleOnly === 1) {
            if (user.auth_method !== 'google') {
                return Response.json({
                    success: false,
                    currentValue: user.google_login_only === 1,
                    error: "כדי להשתמש ב'כניסה מגוגל בלבד', עליך להיות מחובר כעת באמצעות חשבון גוגל."
                }, { status: 400 });
            }
            if (!user.email) {
                return Response.json({
                    success: false,
                    currentValue: user.google_login_only === 1,
                    error: "לא ניתן להפעיל כניסה מגוגל בלבד ללא כתובת אימייל מעודכנת."
                }, { status: 400 });
            }
        }
        finalValue = intentGoogleOnly;
    } 
    else if (key === 'email') {
        const intentEmail = value ? String(value).toLowerCase().trim() : null;
        
        if (user.google_login_only === 1) {
            return Response.json({
                success: false,
                currentValue: user.email || "",
                error: "לא ניתן לעדכן כתובת אימייל בזמן שנעילת גוגל מופעלת. אנא כבה את הנעילה קודם."
            }, { status: 400 });
        }

        if (intentEmail && !intentEmail.includes('@')) {
            return Response.json({
                success: false,
                currentValue: user.email || "",
                error: "כתובת האימייל שהוזנה אינה תקינה."
            }, { status: 400 });
        }
        finalValue = intentEmail;
    }

    try {
        // עדכון השדה הספציפי במסד הנתונים
        await env.DB.prepare(`UPDATE users SET ${key} = ? WHERE phone = ?`)
            .bind(finalValue, user.phone)
            .run();

        // שליפה מיידית מהמסד (מקור אמת יחיד - Single Source of Truth)
        const verifyStmt = await env.DB.prepare(`SELECT ${key} FROM users WHERE phone = ?`).bind(user.phone).first();
        let actualValue = verifyStmt ? verifyStmt[key] : null;

        // התראת אבטחה במידה והופעל google_login_only לראשונה
        if (key === 'google_login_only' && actualValue === 1 && user.google_login_only === 0 && user.email && user.receive_emails !== 0) {
            try {
                const userName = await getNameFromIni(user.phone, env.YEMOT_TOKEN) || "משתמש יקר";
                const userIp = request.headers.get('cf-connecting-ip') || 'לא ידוע';
                await sendSecurityAlert(env, user.email, userName, 'google_only', userIp, null);
            } catch(e) { console.error("Error sending google_only alert", e); }
        }

        // המרת ערכי 0/1 לבוליאנים לצד הלקוח אם השדה הוא receive_emails או google_login_only
        if (key === 'receive_emails' || key === 'google_login_only') {
        ...
        // שגיאה במסד (למשל אם המייל החדש תפוס אצל משתמש אחר)
        const verifyStmt = await env.DB.prepare(`SELECT ${key} FROM users WHERE phone = ?`).bind(user.phone).first();
        let actualValue = verifyStmt ? verifyStmt[key] : null;
        ...
        return Response.json({
            success: false,
            currentValue: actualValue,
            error: "שגיאה במסד הנתונים. ייתכן והאימייל שהזנת כבר בשימוש."
        }, { status: 400 });
    }
}

...
/**
 * פונקציה פנימית מפורטת שבודקת את כל התנאים ומחזירה שגיאות ספציפיות
 */
async function checkEligibility(db, phone, fileName) {
    ...
    // אם הקובץ בכלל לא קיים בטבלה - סימן שהוקלט בטלפון או לפני שדרוג המערכת
    if (!anyUpload) {
        return { 
            allowed: false, 
            message: "לא ניתן למחוק. ההודעה הוקלטה דרך הטלפון או לפני שדרוג המערכת.\nבמידת הצורך יש לפנות להנהלה." 
        };
    }
    ...
    // 4. עכשיו כשאנחנו יודעים שהמשתמש הזה העלה את הקובץ - שולפים את נתוני הצינתוק והזמן
...
export async function handleDeleteMessage(request, env, userIp) {
    ...
    // ==========================================
    // שלב 1: מחיקה בימות המשיח (מופרד כדי למנוע באגים)
    // ==========================================
    ...
    if (yemotDeleted) {
        try {
            ...
            // מחיקה מטבלת ההעלאות + כתיבת הלוג (batch)
            await env.DB.batch([
            ...
            // אם הגענו לפה, הקובץ נמחק בימות אבל הרישום נכשל. נדווח על כך למשתמש
            console.error("DB Log Error: ", dbErr);
            return Response.json({ 
                success: true, 
                message: "ההודעה נמחקה בהצלחה (אך אירעה שגיאת רקע קלה ברישום הלוג)." 
            });
        }
    }
}

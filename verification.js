...
export class VerificationSystem {
    ...
    async requestVerification(phone, ip, intent = 'register') {
        ...
        try {
            ...
            const codeExpiresAtStr = getFutureIsraelTimeForDB(10); 
    ...
    // בקשת איפוס סיסמה ושליחת קוד אימות במייל דרך שירות המיילים החדש
    async requestPasswordReset(identifier, ip, env) {
        ...
        if (user.receive_emails === 0) return { success: false, message: "חשבונך הוגדר שלא לקבל הודעות אימייל מהמערכת. לא ניתן לשלוח קוד איפוס לאימייל שלך. אנא פנה למנהל." };
        ...
        if (blockCheck) {
            ...
            await this.logAction('BLOCKED', user.phone, ip, 'RESET_REJECTED', `ניסיון איפוס נחסם`);
            return { success: false, message: msg };
        }
        ...
        try {
            ...
            // מעבר לשימוש בשירות המיילים החיצוני המעוצב!
            const emailSent = await sendVerificationCodeEmail(env, user.email, displayName, verifyCode, ip);
            
            if (!emailSent) {
                throw new Error("Resend API rejected the email request.");
            }

            await this.logAction('INFO', user.phone, ip, 'RESET_CODE_SENT', `קוד איפוס נשלח למייל`);
    ...
    async blockTarget(type, value, reason, durationValue, durationUnit, ip) {
        ...
        if (existing) {
            await this.db.prepare(`UPDATE verification_blocks SET reason = ?, blocked_until = ? WHERE id = ?`).bind(reason, blockedUntil, existing.id).run();
            return { success: true, message: `החסימה עבור ${value} עודכנה בהצלחה במערכת.` };
        } else {
            await this.db.prepare(`INSERT INTO verification_blocks (block_type, block_value, reason, blocked_until, created_at) VALUES (?, ?, ?, ?, ?)`).bind(type, value, reason, blockedUntil, nowIsraelStr).run();
            return { success: true, message: `החסימה עבור ${value} נוצרה בהצלחה במערכת.` };
        }
    }

    async unblockTarget(target, ip) {
        const result = await this.db.prepare(`DELETE FROM verification_blocks WHERE id = ? OR block_value = ?`).bind(target, String(target)).run();
        if (result.meta && result.meta.changes > 0) return { success: true, message: "החסימה הוסרה בהצלחה ממערכת הניהול." };
        return { success: false, error: "לא נמצאה חסימה התואמת לערך זה." };
    }

    async cleanOldLogs() {
        try {
            const cutOffDateStr = getFutureIsraelTimeForDB(-30 * 24 * 60); 
            await this.db.prepare(`DELETE FROM verification_logs WHERE timestamp < ?`).bind(cutOffDateStr).run();
